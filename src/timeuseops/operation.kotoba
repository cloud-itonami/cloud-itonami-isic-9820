(ns timeuseops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (TimeUseProgrammeAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the TimeUseProgrammeGovernor (:govern)
  ...
  operator/survey coordinator. The approver resumes with
...
(defn build
  ...
  `timeuseops.store/Store`).
  opts:
    :advisor      -- a `timeuseops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TimeUseProgrammeAdvisor inference (the contained intelligence
      ;; node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
      ...
      ;; TimeUseProgrammeGovernor -- independent censor (separate system
      ;; than the advisor).
      (g/add-node :govern
      ...
      ;; can only add caution). HARD governor violations -> HOLD (no
      ;; override).
      (g/add-node :decide
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :household-id (:household-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :household-id (:household-id request) :by (:by approval)}]}
            {:disposition :hold
